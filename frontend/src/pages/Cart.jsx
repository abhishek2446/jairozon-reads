...
const Cart = () => {
   ...
   const {cart, navigate, cartCount, totalCartPrice, updateCart, removeFromCart, axios, setCart, booksData, user,} = useContext(AppContext);
                    ...
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                                <img className="max-w-full h-full object-cover" src={`http://localhost:5000/images/${product.image}`} alt={product.title} />
                            </div>
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select className='outline-none' value={product.quantity} onChange={(e) => {updateCart(product._id, parseInt(e.target.value));}}>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddress ? selectedAddress.address : "No address selected"}</p>
                    ...
                    <select className="w-full border border-gray-300 bg-white px-3 py-2 mt-2 outline-none" onChange={(e) => setPaymentOptions(e.target.value)} value={paymentOptions}>
                ...
                <button className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium  transition">
                    {paymentOptions === "COD" ? "Place Order (Cash On Delivery)" : "Proceed to Payment"}
                </button>
            </div>
        </div>
    )
